...
const AboutHome = () => {
  ...
  return (
    <div className=' min-h-screen bg-gradient-to-b from-[#0f0a0a] via-[#1a1212] to-[#2a1e1e] text-white
       ...
       xl:gap-16 relative'>
        <div className=' w-full order-1 lg:order-2 space-y-8 sm:space-y-12 relative'>
                ...
                {aboutfeature.map((item, i) => (
                    <div key={i} className=' flex flex-col items-center justify-center gap-3 sm:gap-4 transition-transform duration-300 p-4 sm:p-5 hover:translate-x-2'>
                        ...
                        <div className=' text-center'>
                            <h3 className=' text-xl sm:text-2xl font-bold font-cursive'>{t(item.title)}</h3>
                            <p className=' opacity-80 text-sm sm:text-base'>{t(item.text)}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className=' flex flex-wrap gap-4 items-center mt-6 sm:mt-8 px-4 sm:px-0'>
                <Link to='/menu' className=' px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-amber-500 to-orange-600
                 rounded-xl font-bold hover:scale-[1.02] transition-transform duration-300 flex items-center gap-2 sm:gap-3
                  shadow-xl hover:shadow-amber-500/20 group relative overflow-hidden'>
                    ...
                    <span className=' font-cursive text-lg sm:text-xl'>{t("aboutHome.button")}</span>
                </Link>
            </div>
        </div>
