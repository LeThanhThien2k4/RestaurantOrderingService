    ...
    <div className='min-h-screen bg-gradient-to-br from-[#1a120b] via-[#3c2a21] to-[#1a120b] text-amber-50 overflow-hidden relative'>
      ...
      <motion.section initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }}>
        <div className='py-16 px-4 text-center relative'>
          <div className='max-w-4xl mx-auto'>
            ...
            >
              {t('about.title', 'Thức Ăn Nhanh')}
            </motion.h1>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              {t('about.subtitle', 'Crafting unforgettable dining experiences delivered to your doorstep.')}
            </motion.p>
          </div>
        </div>
      </motion.section>
          ...
          {features.map((f, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className='relative h-full flex flex-col bg-[#3c2a21]/90 backdrop-blur-lg rounded-3xl overflow-hidden 
                        border border-amber-600/30 hover:border-amber-500 transition-all duration-300'
                >
                  <div className='relative h-64 overflow-hidden'>
                    <motion.img
                      src={f.img}
                      alt={t(`about.features.${f.id}.title`, f.title)}
                  ...
                  <div className='p-8'>
                    <motion.div className='text-amber-500 mb-4 inline-block' whileHover={{ rotate: 15, scale: 1.1 }}>
                      <Icon className='w-12 h-12 text-amber-500 ' />
                    </motion.div>
                    <h3 className='text-2xl font-bold mb-2 text-amber-100'>
                      {t(`about.features.${f.id}.title`, f.title)}
                    </h3>
                    <p className='text-amber-100/80'>{t(`about.features.${f.id}.text`, f.text)}</p>
                  </div>
                </div>
              </motion.div>
            )
                ...
                >
                  ...
                  >
                    ...
                    <div className='relative z-10 h-full flex flex-col items-center justify-center'>
                      ...
                      >
                        {t(`about.stats.${s.id}.label`, s.label)}
                      </motion.div>
                    </div>
      ...
      <section className='py-16 px-4 md:px-8 relative'>
        <div className='max-w-7xl mx-auto'>
          ...
          >
            {t('about.teamTitle', 'Meet Our')} <span className='text-amber-500'>{t('about.teamHighlight', 'Culinary Artists')}</span>
          </motion.h2>
            ...
            {teamMembers.map((m, i) => (
              ...
              >
                ...
                >
                  ...
                  {/* content: dùng flex-grow thay vì calc để tránh cắt */}
                  <div className='p-8 text-center flex flex-col justify-between flex-grow min-h-0'>
                    <div>
                      <h3 className='text-3xl font-bold mb-2 text-amber-100 break-words'>{m.name}</h3>
                      <p className='text-amber-500 text-lg font-medium font-cursive mb-4'>{t(`about.team.${i}.role`, m.role)}</p>
                      <p className='text-amber-100/80 text-lg font-cursive'>{t(`about.team.${i}.bio`, m.bio)}</p>
                    </div>
