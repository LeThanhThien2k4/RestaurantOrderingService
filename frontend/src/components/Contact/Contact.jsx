...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const message = `
      ...
      ${t("contact.address")}: ${formData.address}
      ${t("contact.dish")}: ${formData.dish}
      ${t("contact.query")}: ${formData.query}
    `;

    const subject = encodeURIComponent(t("contact.emailSubject"));
    const body = encodeURIComponent(message);
    const mailtoUrl = `mailto:2200007423@nttu.edu.vn?subject=${subject}&body=${body}`;

    window.open(mailtoUrl, '_blank');

    toast.success(t("contact.toastOpeningGmail"));
          ...
          <div className='space-y-6'>
            <div className='relative bg-white/5 backdrop-blur-lg rounded-2xl p-6 shadow-2xl transform transition-all
              ...
              <div className='flex items-center mb-4 relative z-10'>
                ...
                <h3 className=' ml-4 text-amber-100 text-xl font-semibold'>{t("contact.headquarters")}</h3>
              </div>
            ...
            <div className='relative bg-white/5 backdrop-blur-lg rounded-2xl p-6 shadow-2xl transform transition-all
              ...
              <div className='absolute inset-0 bg-gradient-to-r from-green-500/10 to-transparent opacity-0
                group-hover:opacity-100 transition-opacity duration-300 rounded-2xl' />
              <div className='flex items-center mb-4 relative z-10'>
                <div className='p-3 bg-gradient-to-br from-green-500/10 to-transparent 0 rounded-xl'>
                  <FiPhone className=' text-green-400 text-2xl animate-ping' />
                </div>
                <h3 className=' ml-4 text-amber-100 text-xl font-semibold'>{t("contact.phoneNumber")}</h3>
              </div>
            ...
            <div className='relative bg-white/5 backdrop-blur-lg rounded-2xl p-6 shadow-2xl transform transition-all
              ...
              <div className='flex items-center mb-4 relative z-10'>
                ...
                <h3 className=' ml-4 text-orange-100 text-xl font-semibold'>{t("contact.email")}</h3>
              </div>
              <div className='pl-12 relative z-10'>
                <p className='text-orange-100 font-light text-lg'>2200007423@nttu.edu.vn</p>
              </div>
            </div>
          </div>
          ...
          <div className='relative bg-white/5 backdrop-blur-lg rounded-2xl p-6 shadow-2xl animate-slide-in-right border-2
            ...
            <form onSubmit={handleSubmit} className='space-y-6 relative z-10'>
              {contactFormFields.map(({ label, name, type, placeholder, pattern, Icon }) => (
                <div key={name}>
                  <label className='block text-amber-100 text-sm font-medium mb-2'>{t(`contact.form.${name}`)}</label>
              ...
              <div>
                <label className='block text-amber-100 text-sm font-medium mb-2'>{t("contact.yourRequest")}</label>
                <div className='relative'>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    className='w-full pl-10 pr-4 py-3 bg-white/10 border-2 border-amber-500/30
                      text-amber-50 focus:ring-2 focus:ring-amber-500 focus:border-transparent
                      placeholder-amber-200/50'
