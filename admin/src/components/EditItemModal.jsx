...
const EditItemModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    rating: 0,
    hearts: 0,
    image: null,     // file local nếu chọn mới
    preview: ''      // ảnh preview (Cloudinary URL hoặc local blob)
  });
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        ...
        image: null,
        preview: item.imageUrl || '' // backend đã trả link Cloudinary
      });
    }
  }, [item, isOpen]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (formData.image) {
        // Nếu chọn ảnh mới → upload file
        payload.append('image', formData.image);
      } else {
        // Nếu không → giữ nguyên ảnh cũ
        payload.append('imageUrl', formData.preview);
      }
